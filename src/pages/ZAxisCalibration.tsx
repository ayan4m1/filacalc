...
export default function ZAxisCalibration() {
  const {
    ...
    validate: useCallback((vals: ZAxisCalibrationForm) => {
      ...
      if (vals.layerHeight <= 0) {
        result.push('Layer height must be greater than zero.');
      }
  ...
  return (
    <Calculator>
      <Calculator.Heading title="Z-Axis Calibration" />
      <Calculator.Description>
        <p>
          Use this calculator to calibrate your layer heights based on your
          Z-axis hardware.
        </p>
        <p>
          The goal is to use a layer height which is evenly divisble by the
          smallest amount of motion your Z-axis is capable of producing.
        </p>
        <p>
          If you do not do this, you can expect to see various print issues,
      ...
      {Boolean(results) && (
        <Calculator.Results results={results.results}>
